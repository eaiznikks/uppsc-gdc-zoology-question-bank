...
UNIT_FILE = ROOT / "Unit-01-Non-Chordates.md"
...
def extract_answers(text: str) -> list[tuple[str, str]]:
    pattern = re.compile(r"(?ms)^###\s+((?:Q|S)\d+\.\s+.+?)\n\n(.*?)(?=^###\s+(?:Q|S)\d+\.|\Z)")
...
def main() -> int:
    ...
    for heading, block in answers:
        ...
        if not (MIN_WORDS <= wc <= MAX_WORDS):
            errors.append(f"{heading}: answer word count {wc}, expected {MIN_WORDS}-{MAX_WORDS}.")
    ...
    if errors:
        print("Unit 1 validation failed:")
        for err in errors:
            print(f"- {err}")
        return 1

    print(f"Unit 1 validation passed: {len(answers)} answers, all {MIN_WORDS}-{MAX_WORDS} words, required sections present.")
    return 0
